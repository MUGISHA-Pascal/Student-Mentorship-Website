        ...
        <div>
            {currentScreen === 'sendOTP' ? (
                ...
                <OTPVerification
                    ...
                    setOtp={setOtp}
                    isSubmitting={isSubmitting}
                    handleBack={() => { setCurrentScreen('sendOTP') }}
                />
            ) : (
                <NewPassword
                    handleNewPassword={handleNewPassword}
                    isSubmitting={isSubmitting}
                    handleBack={() => { setCurrentScreen('otpVerification') }}
                />
            )}
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="w-3/5 h-3/5 bg-white p-6 rounded-lg shadow-lg flex flex-col items-center justify-center">
                        <h2 className="text-3xl font-bold mb-4">Password successfully recovered!</h2>
                        <p className="font-semibold text-gray-500 mb-20">Return to the login page and use your new password.</p>
                        <button

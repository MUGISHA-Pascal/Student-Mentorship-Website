...
const UploadModal = ({ isOpen, onClose, userId, onCourseUploaded }: { isOpen: boolean; onClose: () => void; userId: string | null; onCourseUploaded: () => void }) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent>
                ...
                {step === 1 && (
                    <div
                        className={`border-2 border-dashed rounded-lg p-6 ${dragActive ? 'border-blue-500' : 'border-gray-300'}`}
                                ...
                                <li key={index} className="flex items-center justify-between border p-2 rounded">
                                    <div className="flex-1">
                                        ...
                                        {file.status === 'uploading' && (
                                            <p className="text-xs text-gray-400">{file.timeRemaining}</p>
                                        )}
                                    </div>
                ...
                <DialogFooter className="mt-4 flex justify-between">
                    ...
                    {step === 1 && (
                        <Button
                            ...
                            className={uploadedFiles.some((file) => file.status === 'uploading') ? 'opacity-50' : ''}
                        >
                            Next
                        </Button>
                    )}
